...
**Loop**:
A closed perception → decision → action → learning cycle, with explicit frequency, inputs, outputs, permissions, and stopping criteria. Domain: time and internal state. It reads balances from `Reconciliation`, not from `MarketGraph`; it only crosses the graph when costeo de `OpportunityCandidate` requires a route. Never mutates `MarketGraph`; never uses untyped agent output.
...
_Agnosticismo temporal / Snapshot inmutable (resuelto 2026-08-27): el grafo no sabe si opera en presente o pasado; su única función es calcular ruta y beneficio neto con los pesos actuales. En replay, el Harness levanta un `MarketGraph` efímero (nunca muta el de producción) inyectando pesos históricos. El grafo nunca almacena capital ni intenciones (`Loop` mantiene balances vía `Reconciliation`, no vía grafo). Solo se cruza con `Loop` al costear un `OpportunityCandidate`.
...
**Risk Engine**:
The deterministic authority that approves, rejects, reduces, or blocks every `OrderIntent`. It emits a `RiskDecision` (`packages/contracts/src/risk.ts`) with `decision: RiskDecisionOutcome` (`APPROVE` · `REJECT` · `REDUCE_SIZE` · `EXIT_ONLY` · `CANCEL_ONLY` · `CASH_ONLY` · `HALT_SYSTEM` — see `reason-codes.ts`). The defensive outcome `HALT_SYSTEM` is a risk *dictamen*, not a mode: it instructs the orchestrator to enter `HALT` (`SystemMode`) / `HALT` (`StateName` terminal), but the Engine never sets `SystemMode` directly. It can trigger the kill switch; it is mandatory — no `OrderIntent` exists without its approval. Authority is isolated from agents (ADR-0003); agents never receive `APPROVE_RISK`.
_Nomenclature rule (resolved 2026-08-27): `HALT_SYSTEM` (outcome) ≠ `HALT` (mode/state). The transition from `RiskDecision` to `SystemMode` is owned by the `StateGraph` / orchestrator, never by the Risk Engine. `RiskDecisionOutcome` and `SystemMode` share lexical roots (`CANCEL_ONLY`, `CASH_ONLY`) by explicit namespace design; never infer one from the other without `mapRiskToMode`.
_Avoid_: advisory risk agent, optional layer, or conflating the dictamen (`HALT_SYSTEM`) with the global permission state (`HALT`).
...
_Protocolo de ceguera parcial (resuelto 2026-08-27): si `WebSocket` cae con un `partial fill` activo, el `StateGraph` transiciona inmediatamente a modo defensivo (`CANCEL_ONLY_MODE` / `OBSERVE_ONLY_MODE` / `HALT` si REST falla). El `Loop` congela; `Risk Engine` rechaza nuevos `OrderIntent`. `Infra` activa REST de emergencia; si confirma el remanente vivo, se emite cancelación inmediata; solo tras cuadrado matemático exacto se retorna a `NORMAL`. El `Execution Engine` destruye cualquier `OrderIntent` con `expiresAtMs` pasado sin reconsultar al `Risk Engine` (fail closed).
_Avoid_: trusting internal state without external verification; executing expired approvals; allowing a loop to generate opportunities while reconciliation is unresolved.
...
_Nomenclature rule (resolved 2026-08-27): three explicit namespaces — no lexical fusion. `SystemMode` (modes.ts) has NO suffix; defensive states (`StateName`, stategraph.ts) carry `_MODE` (`CASH_ONLY_MODE`, `CANCEL_ONLY_MODE`, `REDUCE_ONLY_MODE`, `DEGRADED_MODE`) plus terminal `HALT`; `RiskDecisionOutcome` (reason-codes.ts) has its own namespace (`APPROVE`, `REJECT`, `REDUCE_SIZE`, `HALT_SYSTEM`, `CANCEL_ONLY`, `CASH_ONLY`). The literal overlap (`CANCEL_ONLY` / `CASH_ONLY`) is intentional namespace separation; transition from outcome to mode must go through an explicit deterministic mapping (`mapRiskToMode`), never lexical inference. `OBSERVE_ONLY` is the primary defensive mode (replacing the Alpha spec's "degraded"); the degraded classification lives exclusively in `DATA_QUALITY_STATES`.
